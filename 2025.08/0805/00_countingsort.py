def counting_sort(DATA, TEMP, K):
    ...
    # TEMP : 정렬 결과(배열, 리스트)
    # K : 정렬 대상 배열 안에 들어있는 정수 중 최댓값
    # K == 카운트 배열의 크기

    # 파이썬은 인덱스 0부터 시작하므로 늘 1을 더해줘야 함
    COUNT = [0] * (K+1)
    ...
    # 각 원소가 정렬 후에 들어갈 자리 위치를 계산
    # 어떤 숫자 x보다 작은 숫자가 몇 개 있는지 알고 있다면
    ...
    # 정렬 후 위치가 된다.
    # 뒤에서부터 확인하는 이유는? 안정 정렬을 위해서(원래 순서 보장)
    for i in range(len(DATA)-1, -1, -1):
        # DATA[i]에 있는 숫자의 정렬 후 위치는 어디인가?
